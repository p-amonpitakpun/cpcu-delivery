...
def create_OccupancyGrid(slam, name='', size=500):
    ...
    with ProgressBar(max_value=len(V)) as bar:
        for i, v in enumerate(V):
            ...
            img = cv2.putText(img, f'{v.point}', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 100))
...
def compute(log, slam_type='graph', optimized=True, name='', run_graph=True, size=500, progress=True, real=True):
    ...
    if real:
        ...
        with ProgressBar(max_value=1050) as bar:
            for i, data in enumerate(log['data'][: 1050]):
                slam, first_pos = process_realdata(slam, data['realpos'], data['scanner'], first_pos)
                bar.update(i)
    else:
        last_odom = None
        if progress:
